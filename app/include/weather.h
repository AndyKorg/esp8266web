...
#define WEATHER_TIMEOUT			(1000*60)						//минута
...
//Настройки клиента
typedef struct{
	char APIkey[64];					//ключ API получаемый от weather.org
	uint32 city;						//Индекс населенного пункта, если 0 то используется долгота-широта
	char lat[13];						//Широта (Latitude) в виде строки-числа с точкой "-11.2222"
	char lon[13];						//Долгота (Longitude)
